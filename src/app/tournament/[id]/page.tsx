            ...
            <div className={styles.contentWrapper}>
                                        ...
                                        <div style={{ flex: 1 }}>
                                            <p style={{
                                                ...
                                                {player.userId === tournament.hostId && (
                                                    <span style={{ color: '#FFD700', fontSize: '0.75rem', marginLeft: '8px' }}>(hote)</span>
                                                )}
                                                {player.userId.startsWith('guest_') && (
                                                    <span style={{ color: '#E67E22', fontSize: '0.75rem', marginLeft: '8px' }}>(guest)</span>
                                                )}
                                            </p>
                                        </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-sm)' }}>
                                ...
                                <button
                                    ...
                                    style={{
                                        ...
                                        gap: '8px',
                                        padding: 'var(--spacing-md)',
                                        background: canStartSetup ? '#FFD700' : 'rgba(255, 215, 0, 0.3)',
                                        border: '3px solid #333333',
                                        borderRadius: 'var(--radius-md)',
                                        color: 'var(--color-text-dark)',
                                        fontWeight: 700,
                ...
                {tournament.status === 'team_setup' && (
                    <>
                                            ...
                                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                                {team.players.map(player => (
                                                    <span
                                                        ...
                                                        style={{
                                                            padding: '4px 8px',
                                                            background: player.userId.startsWith('guest_') ? '#F39C12' : '#2ECC71',
                                                            borderRadius: 'var(--radius-sm)',
                                                            color: 'white',
                                                            fontSize: '0.75rem',
                                                            fontWeight: 600
                        ...
                        {/* Unassigned Players */}
                        {tournament.players.filter(p => !tournament.teams.some(t => t.players.some(tp => tp.userId === p.userId))).length > 0 && (
                            <div style={{ marginBottom: 'var(--spacing-lg)' }}>
                                <p style={{ fontSize: '0.875rem', color: 'var(--color-text-dark)', opacity: 0.6, fontWeight: 600, marginBottom: 'var(--spacing-sm)' }}>
                                    Joueurs non assignes
                                </p>
                                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                    {tournament.players
                                        .filter(p => !tournament.teams.some(t => t.players.some(tp => tp.userId === p.userId)))
                                        .map(player => (
                                            <span
                                                key={player.userId}
                                                style={{
