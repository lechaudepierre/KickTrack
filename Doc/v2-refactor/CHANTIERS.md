...
`getUserGames(targetUserId, 200)` : le profil calcule **toutes** ses
statistiques depuis cette liste. Au-delà de la limite, les parties les plus
anciennes n'entraient dans aucun chiffre — sans que rien ne le signale.

**Ce n'était pas théorique.** Relevé sur les 772 parties terminées :

| | parties | conséquence |
|---|---|---|
| lechauvepierre | **256** | 56 parties invisibles dans son profil |
| 2ᵉ joueur | **223** | 23 invisibles |
| 3ᵉ | 192 | complet |
| médiane | 4 | complet |

**Et la limite ne protégeait presque personne.** Une partie pèse 3 848 octets
en moyenne. Pour lechauvepierre, plafonner économisait 190 Ko sur 877 — au prix
de statistiques fausses. Le profil médian, lui, a 4 parties et 16 Ko : la limite
ne l'a jamais concerné. On payait une correction sur les gros joueurs pour un
gain nul sur les autres.

**Un second plafond, caché.** `getUserGames` contenait
`Math.min(limitCount + 30, 300)` : tout `limitCount` supérieur à 270 était
silencieusement ramené à 300. Relever la limite de l'appelant à 500 n'aurait
donc **rien changé** — le vrai correctif était là.

Pire : la fonction téléchargeait 230 parties, puis en jetait 30 avec un
`.slice(0, 200)`. Trente parties payées et non utilisées, à chaque ouverture de
profil.

**Vérifié après correction**, sur le plus gros profil réel :

| limite | parties rendues | poids |
|---|---|---|
| 230 (avant) | 230, puis coupées à 200 | 799 Ko |
| 530 (après) | **256 — toutes** | 877 Ko |

**78 Ko de plus pour des chiffres justes.**

Le plafond suit désormais la demande : c'est à l'appelant de savoir ce qu'il
charge, pas à cette fonction de le corriger dans son dos.

ATTENTION: au-delà d'environ 500 parties pour un même joueur, ce n'est plus la
limite qu'il faudra relever mais la requête qu'il faudra borner par saison,
côté serveur — le `seasonId` du chantier 3.8 est déjà en place pour ça.
